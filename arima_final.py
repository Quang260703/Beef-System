...
def analyze_data(df, col):
    ...
    stepwise_model = auto_arima(df['Real_Price'], seasonal=False, trace=True)
    print(stepwise_model.order) # (2, 1, 0)
...
def main():
    ...
    # Assuming your DataFrame is named df and 'Date' is already set as the index
    ...
    # Combine test dates, actuals, and predictions into a DataFrame
    comparison_df = pd.DataFrame({
        'Date': dates_test,
        'Actual': test['Real_Price'],
        'Predicted': original_pred
    })

    # Save to CSV
    comparison_df.to_csv('arima_forecast_comparison.csv', index=False)

    print("CSV saved: arima_forecast_comparison.csv")
